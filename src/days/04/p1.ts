...
function isPairRedundant(elves: Pair<Section>): boolean {
  const [elfA, elfB] = elves.map(createBitsetForElf);
  return elfA.and(elfB).equals(elfA) || elfB.and(elfA).equals(elfB);
}
